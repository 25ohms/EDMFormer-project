...
def _read_gcs_npy(uri: str) -> np.ndarray:
    bucket_name, blob_name = _parse_gcs_uri(uri)
    client = _get_gcs_client()
...
def _list_gcs_npy_stems(prefix_uri: str) -> set[str]:
...
class Dataset(Dataset):
    def get_ids_from_dir(self, dir_path: str):
        if _is_gcs_path(dir_path):
            return _list_gcs_npy_stems(dir_path)
    ...
    def __init__(
        ...
        for dataset_abstract_item in dataset_abstracts:
            ...
            if adapter is not None:
                raise ValueError(
                    "Adapter-based datasets are not supported in edmformer_gcs_dataset."
                )
            ...
            for _ in range(dataset_abstract_item["multiplier"]):
                self.valid_data_ids.extend(valid_data_ids)

            self.init_segments(label_path=label_path, internal_tmp_id=internal_tmp_id)

        logger.info(f"{uniq_id_nums} valid data ids, {len(self.valid_data_ids)} total")
        rng = np.random.default_rng(42)
        rng.shuffle(self.valid_data_ids)

    def init_segments(self, label_path, internal_tmp_id):
        if _is_gcs_path(label_path):
            label_text = _read_gcs_text(label_path)
            label_lines = label_text.splitlines()
        else:
        ...
        for line in label_lines:
            ...
            self.time_datas[hybrid_id] = list(map(float, self.time_datas[hybrid_id]))
    ...
    def __getitem__(self, idx):
        try:
            ...
            if adapter_str is not None:
                raise ValueError("Adapter-based datasets are not supported here.")

            embd_list = []
            embd_dirs = self.input_embedding_dir[internal_tmp_id].split()
            for embd_dir in embd_dirs:
                if not _is_gcs_path(embd_dir) and not Path(embd_dir).exists():
                    raise FileNotFoundError(
                        f"Embedding directory {embd_dir} does not exist"
                    )
                if _is_gcs_path(embd_dir):
                    tmp = _read_gcs_npy(f"{embd_dir.rstrip('/')}/{utt}.npy").squeeze(axis=0)
                else:
            ...
            for j in np.flatnonzero(keep_boundarys):
                true_boundary[self.time2frame(local_times[j])] = 1
            ...
            for j in np.flatnonzero(keep_boundarys):
                true_function[
                    last_pos : self.time2frame(local_times[j]),
                    int(local_labels[j - 1]),
                ] = 1
                true_function_list.append(int(local_labels[j - 1]))
                last_pos = self.time2frame(local_times[j])
                msa_info.append(
                    (
                        float(max(local_times[j - 1], time_L)),
                        str(self.id_to_label[int(local_labels[j - 1])]),
                    )
                )
    ...
    def collate_fn(self, batch):
        try:
            ...
            for i, item in enumerate(batch):
                ...
                input_embeddings[i, : item["input_embedding"].shape[0]] = item[
                    "input_embedding"
                ]
                masks[i, : item["mask"].shape[0]] = item["mask"][:max_sequence_length]
                true_boundaries[i, : item["true_boundary"].shape[0]] = item[
                    "true_boundary"
                ][:max_sequence_length]
                widen_true_boundaries[i, : item["widen_true_boundary"].shape[0]] = (
                    item["widen_true_boundary"]
                )[:max_sequence_length]
                true_functions[i, : item["true_function"].shape[0]] = item[
                    "true_function"
                ][:max_sequence_length]
                ...
                boundary_mask[i, : item["mask"].shape[0]] = item.get(
                    "boundary_mask", np.zeros(item["mask"].shape[0], dtype=bool)
                )[:max_sequence_length]
                function_mask[i, : item["mask"].shape[0]] = item.get(
